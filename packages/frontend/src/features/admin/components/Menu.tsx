...
type MenuProps = {
    onSelect: (section: 'events' | 'sculptures' | 'sculptors' | 'users') => void;
    selectedSection: 'events' | 'sculptures' | 'sculptors' | 'users';
};
        ...
        <div className="flex justify-center items-center my-8"> {/* Asegurando que esté centrado */}
            <div className="flex space-x-6 justify-center items-center"> {/* Espaciado entre botones */}
                {menuItems.map((item) => (
                    <button
                        key={item.value}
                        onClick={() => onSelect(item.value as 'events' | 'sculptures' | 'sculptors' | 'users')}
                    ...
                    >
                        <span
                            className="absolute inset-0 flex items-center justify-center text-lg z-10 backdrop-blur-lg">
                            {item.label}
                        </span>
